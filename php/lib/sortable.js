...
(function (factory){
    ...
    function Sortable(el, options){
        ...
        _on(el, 'end', options.onEnd);
        _on(el, 'orderchange', options.onOrderChange);
    ...
    Sortable.prototype = {
        ...
        _onTapStart: function (evt/**Event|TouchEvent*/){
            ...
            target.dispatchEvent(_createEvent('start', target));
        ...
        _onDragOver: function (evt/**Event*/){
            ...
            if( !_silent && (activeGroup === this.options.group) && (evt.rootEl === void 0 || evt.rootEl === this.el) ){
                ...
                if( el.children.length === 0 || el.children[0] === ghostEl || (el === evt.target) && _ghostInBottom(el, evt) ){
                    ...
                    dragEl.dispatchEvent(_createEvent('orderchange', dragEl));
                    if( lastEl !== target ){
                        lastEl = target;
                        lastCSS = _css(target);
        ...
        _onDrop: function (evt/**Event*/){
            ...
            evt.target.dispatchEvent(_createEvent('end', evt.target));
