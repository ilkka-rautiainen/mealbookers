...
class SilinteriImport extends AmicaImport implements iImport
{
    ...
    protected function saveOpeningHours()
    {
        ...
        if (count($processed_lines) != 2) {
            Logger::warn(__METHOD__ . " {$this->restaurant_id} opening days got "
                . count($processed_lines) . " lines instead of 2");
            return;
        }

        $imploded = implode("|", $processed_lines);
        Logger::debug(__METHOD__ . " $imploded");
        if (!preg_match("/ma[\s]*\\-[\s]*pe[\s]+kl[\S]*\\.?[\s]*(([0-9]|0[0-9]|1[0-9]|2[0-3])[:.][0-5][0-9])[\s]*\\-[\s]*(([0-9]|0[0-9]|1[0-9]|2[0-3])[:.][0-5][0-9])\\|"
